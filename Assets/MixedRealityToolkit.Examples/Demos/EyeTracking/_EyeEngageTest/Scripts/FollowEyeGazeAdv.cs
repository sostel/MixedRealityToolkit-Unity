...
namespace Microsoft.MixedReality.Toolkit.Examples.Demos.EyeTracking
{
    ...
    /// </summary>
    public class FollowEyeGazeAdv : MonoBehaviour
    {
        ...
        [SerializeField]
        private float minLookAwayThresh = 5;

        private void Update()
        {
            if (keepFacingUser)
            {
                this.gameObject.transform.LookAt(CameraCache.Main.transform.position);
            }

            var eyeGazeProvider = CoreServices.InputSystem?.EyeGazeProvider;
            if (eyeGazeProvider != null)
            {
                Vector3 dir1 = gameObject.transform.position - CameraCache.Main.transform.position;
                Vector3 dir2 = eyeGazeProvider.GazeDirection.normalized;
                
                if (Mathf.Abs(Vector3.Angle(dir1, dir2)) < minLookAwayThresh)
                {
                    return;
                }
